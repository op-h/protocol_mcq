...
lectures.push({
  id: "exam-review",
  title: "Protocol Exam Review",
  label: "Exam Review",
  questions: [
    mcq("What happens at each router to the Time to Live (TTL) value of a datagram?", ["It is reset to the default value", "It remains unchanged", "It is incremented by one", "It is decremented by one", "It is doubled"], 3, "exam"),
    mcq("What is a key characteristic of Private IP addresses?", ["They are globally unique on the Internet", "They must be purchased from an ISP", "They are 128 bits long", "They are not routable through the Internet", "They are only used for IPv6 transition"], 3, "exam"),
    mcq("What is the primary characteristic of communication at the internet layer?", ["Source-to-destination communication", "Physical-to-logical mapping", "Node-to-node error correction", "Process-to-process communication", "Port-to-port multiplexing"], 0, "exam"),
    mcq("Which part of the IPv4 header is used to control fragmentation?", ["Type of Service", "Header Checksum", "Options", "Flags", "Padding"], 3, "exam"),
    mcq("Which of the following is a main benefit of IPv6 over IPv4?", ["Smaller address space for faster lookups", "Support for broadcast only", "Variable-length fixed headers", "Larger address space and simpler auto-configuration", "Mandatory use of NAT"], 3, "exam"),
    mcq("What is the range for the IPv4 header length in bytes?", ["20 to 60 bytes", "40 to 100 bytes", "32 to 64 bytes", "10 to 30 bytes", "5 to 15 bytes"], 0, "exam"),
    mcq("When is the Tunneling strategy specifically used during the transition to IPv6?", ["When an IPv6 packet must pass through a region that uses IPv4", "When a sender uses IPv4 and the receiver understands only IPv6", "To encrypt data for private networks", "To eliminate the need for a 128-bit address", "When the entire Internet has already moved to IPv6"], 0, "exam"),
    mcq("What defines an Anycast address in IPv6?", ["A packet broadcast to all nodes on the Internet", "A packet delivered to a single intended recipient only", "A packet delivered to every member of a group", "A private address used inside a local network", "A packet delivered to only one member of a group, the most reachable one"], 4, "exam"),
    mcq("What problem does NAT primarily solve for small businesses?", ["Fragmentation of large packets", "Slow internet speeds", "Excessive hardware costs", "Lack of contiguous public IP address blocks", "Data encryption issues"], 3, "exam"),
    mcq("What is EUI-64 used for in IPv6?", ["Compressing leading zeros", "Encrypting the payload of a datagram", "Translating IPv6 headers to IPv4 headers", "Mapping a 48-bit physical address to a 64-bit interface identifier", "Routing packets between different LANs"], 3, "exam"),
    mcq("In IPSec transport mode, which part of the packet remains unprotected?", ["ESP trailer", "Authentication header", "Payload", "Transport layer header", "Original IP header"], 4, "exam"),
    mcq("Which DHCP message is sent by the server to confirm that the client may use the offered IP address?", ["DHCP Discover", "DHCP NAK", "DHCP Request", "DHCP Release", "DHCP Ack"], 4, "exam"),
    mcq("Which forwarding method pre-computes all routing and adjacency information to enable line-rate packet forwarding?", ["SDN Flow Switching", "Fast Switching", "Process Switching", "Cisco Express Forwarding (CEF)", "MPLS Label Switching"], 3, "exam"),
    mcq("Why can DHCP Discover and DHCP Request messages not cross routers without a relay agent?", ["They are broadcast messages that routers do not forward", "They require encryption before forwarding", "They use a non-IP encapsulation format", "They exceed the MTU size of most networks", "They use invalid IP headers"], 0, "exam"),
    mcq("Which forwarding method processes every packet using the CPU, making it the slowest?", ["Process Switching", "Cisco Express Forwarding (CEF)", "MPLS Switching", "SDN Flow Switching", "Fast Switching"], 0, "exam"),
    mcq("What prevents packets from circulating endlessly in a network?", ["Time to Live (TTL)", "Checksum", "Flags", "Identification", "Fragment Offset"], 0, "exam"),
    mcq("In the DHCP process, which message does the client send first?", ["DHCP Ack", "DHCP Discover", "DHCP Request", "DHCP Offer", "DHCP Release"], 1, "exam"),
    mcq("In packet forwarding, which step determines the next-hop router or exit interface?", ["TTL adjustment", "Longest Prefix Match lookup", "Encapsulation", "Validation", "Queue scheduling"], 1, "exam"),
    mcq("Which of the following is a primary purpose of ICMP?", ["Managing routing tables", "Reporting errors and providing feedback", "Ensuring reliable delivery of IP packets", "Encrypting IP headers", "Assigning IP addresses dynamically"], 1, "exam"),
    mcq("When a router performs a Longest Prefix Match (LPM), what is it specifically trying to determine?", ["The most specific matching network prefix for the destination IP", "The next hop with the highest bandwidth", "The shortest physical path to the destination", "The route with the lowest administrative distance", "The interface with the lowest congestion"], 0, "exam"),
    mcq("What characterizes the Backbone in Hierarchical OSPF?", ["It is a local area that does not communicate with others", "It only runs distance vector protocols", "It is the primary area that connects different local areas", "It is only used in small organizations", "It is only used to connect to ISP"], 2, "exam"),
    mcq("What is the primary metric used by the RIP protocol for path selection?", ["Load", "Bandwidth", "Cost", "Hop count", "Delay"], 3, "exam"),
    mcq("Static routing is best suited for which networks?", ["Networks with 10-15 routers", "Cloud data centers", "Mobile networks", "ISPs", "Peer-to-peer systems"], 0, "exam"),
    mcq("What distinguishes iBGP from eBGP?", ["iBGP requires manual configuration of all paths", "eBGP is used only for local area networks", "iBGP is a distance vector protocol, while eBGP is link state", "iBGP is used for internal routing within the same AS, while eBGP connects different ASs", "Both iBGP and eBGP perform the same function"], 3, "exam"),
    mcq("What does a routing table consist of?", ["A list of all connected users", "All possible routes to every destination", "Only the best routes for every destination", "Every MAC address in the network", "Only neighbors networks"], 2, "exam"),
    mcq("What is the maximum number of hops allowed for the RIP protocol?", ["15", "10", "Unlimited", "30", "25"], 0, "exam"),
    mcq("In Path Vector Routing, what is the role of the Speaker Node?", ["It handles all local traffic within the AS", "It runs OSPF protocol", "It advertises network reachability information to neighboring speaker nodes in other ASs", "It manually configures static routes", "It only listens to Link State updates"], 2, "exam"),
    mcq("A collection of routers under a common administration, like a company or ISP, is known as a(n):", ["Group", "Autonomous System (AS)", "Subnet", "Speaker Node", "Gateway"], 1, "exam"),
    mcq("What happens in RIP when the Route timeout or Expiration Timer of 180 seconds expires?", ["The traffic is redirected to a static route", "The router reboots", "The route is immediately deleted", "Nothing happens", "The route is set to a hop count of 16, unreachable"], 4, "exam"),
    mcq("What is the goal of the Border Gateway Protocol (BGP)?", ["To act as a distance vector protocol inside an AS", "To connect the link state to distance vector", "To find the optimal shortest path based on bandwidth", "To find any path that is loop-free and compliant with defined policies", "To eliminate the need for the Transport layer"], 3, "exam"),
    mcq("What is the main function of the Transport Layer?", ["Managing web page formatting", "Defining the hardware address of a device", "End-to-end data delivery and logical communication between processes", "Physical transmission of bits", "Encryption and translation"], 2, "exam"),
    mcq("What is a major disadvantage of using UDP over TCP?", ["It receives data in order", "It offers no flow control or error recovery mechanism", "It is slower than TCP", "It has a larger header size", "It requires complex handshaking"], 1, "exam"),
    mcq("What is the difference between Well-known and Registered ports?", ["Well-known use UDP; Registered use TCP", "There is no difference", "Well-known are for private use; Registered are for the Internet", "Well-known are 0-1023 and controlled by IANA; Registered are 1024-49151 and can be registered to prevent duplication", "Well-known are public while Registered are private"], 3, "exam"),
    mcq("Which port does the Telnet protocol use?", ["24", "23", "22", "21", "25"], 1, "exam"),
    mcq("Which protocol allows a user to log in to a remote host and take control as if they were sitting at that machine?", ["HTTP", "DNS", "SMTP", "DHCP", "Telnet"], 4, "exam"),
    mcq("What is the default port used by the HTTPS protocol?", ["433", "25", "443", "21", "80"], 2, "exam"),
    mcq("Which protocol is specifically designed for downloading or uploading files between a client and server?", ["Telnet", "FTP", "DNS", "SMTP", "DHCP"], 1, "exam"),
    mcq("A Uniform Resource Locator (URL) defines which four things?", ["Metric, Network, Route", "Protocol, Host, Port, Path", "Username, Password, Domain, Browser", "Name, Date, Size, Type", "IP, MAC, Subnet, Gateway"], 1, "exam"),
    mcq("Which range of port numbers is known as Well-known ports?", ["49152 to 65535", "0 to 1000", "1024 to 49151", "0 to 1023", "80 to 443"], 3, "exam"),
    mcq("Which protocol is used to handle sending email messages from clients?", ["SMTP", "IMAP", "DHCP", "POP3", "HTTP"], 0, "exam"),
    mcq("Which protocol requires the sender to send one frame and then stop until it receives a confirmation from the receiver?", ["ALOHA Protocol", "Sliding Window Protocol", "Selective-Repeat Protocol", "Go-Back-N Protocol", "Stop-and-Wait Protocol"], 4, "exam"),
    mcq("Which function is explicitly listed as a responsibility of the Logical Link Control (LLC) sublayer?", ["Flow Control", "Dynamic Channel Allocation", "Physical Addressing", "Quality of Service (QoS)", "Routing table management"], 0, "exam"),
    mcq("In the context of the Data Link Layer, what does Flow Control refer to?", ["Restricting the amount of data a sender can send before receiving an acknowledgment", "Finding the fastest route through the Internet", "Converting digital signals to analog signals", "Assigning IP addresses to new devices", "Encrypting data frames for security"], 0, "exam"),
    mcq("Which protocol is used to map logical IP addresses to physical MAC addresses?", ["STP", "DNS", "ARP", "DHCP", "L2TP"], 2, "exam"),
    mcq("What is the primary purpose of the Spanning Tree Protocol (STP)?", ["To manage wireless frequency channels", "To translate domain names into IP addresses", "To prevent broadcast storms caused by switching loops", "To assign dynamic IP addresses", "To compress data frames"], 2, "exam"),
    mcq("What is the difference between CSMA/CD and CSMA/CA?", ["CSMA/CD uses a master-slave configuration", "CSMA/CA does not require sensing the medium", "CSMA/CD detects collisions; CSMA/CA tries to avoid them", "CSMA/CD is random access, while CSMA/CA is controlled access", "CSMA/CD is only used in wireless networks"], 2, "exam"),
    mcq("What is the main goal of a Virtual Private Network (VPN), which can be enabled by protocols like L2TP?", ["To manage switching loops in local networks", "To replace the physical layer of the Internet", "To provide private communications within the public Internet infrastructure", "To eliminate the need for MAC addresses", "To increase the speed of public web browsing"], 2, "exam"),
    mcq("How do Sliding Window protocols like Go-back-N improve efficiency compared to Stop-and-Wait?", ["By allowing multiple frames to be in transition, outstanding, at once", "By using a dedicated point-to-point link", "By removing the need for a receiver buffer", "By eliminating error detection mechanisms", "By waiting for an acknowledgment after every bit"], 0, "exam"),
    mcq("In the Go-Back-N protocol, what does the receiver do if a single frame k is missing?", ["It only requests the missing frame k", "It sends a broadcast request for the missing frame", "It uses error correction to fix frame k", "It discards all subsequent frames after k and sends no acknowledgments", "It buffers frame k+1 while waiting for k"], 3, "exam"),
    mcq("In Random Access methods, what happens if more than one station tries to send data at the same time?", ["The stations must consult each other to authorize transmission", "An access conflict, collision, occurs, and frames are destroyed or modified", "One station is assigned as the master station", "The network automatically schedules them", "The medium becomes a dedicated point-to-point link"], 1, "exam")
  ]
});
